...
-- ============================================================
-- ┌──────────────────────────────────────────────────────────┐
-- │ MAJ B5 (30 mai 2026) · Autorisation granulaire par         │
-- │ catégorie — point 3/6 surface 2/3 (RPC évènement).         │
-- │ Doc FAIT FOI Conception-Autorisation-Referent-Categorie-v1 │
-- │ (md5 2c99a098), §4.2 + §6. Deux changements, fichier        │
-- │ complet (jamais un patch) :                                 │
-- │  (0) garde rôle : coach RETIRÉ (D8, sql/63) -> la PORTE =    │
-- │      admin OR bureau OR referent.                           │
-- │  (1bis) bornage catégoriel AVANT tout INSERT : chaque       │
-- │      équipe entrante (p_equipe_id + p_equipes_engagees[])   │
-- │      doit être dans une catégorie autorisée                 │
-- │      (puis_je_ecrire_categorie + _b5_categorie_de_equipe,   │
-- │      sql/64 + sql/65). admin/bureau transverses ; referent  │
-- │      borné. Refus = exception AVANT effet de bord (anti-    │
-- │      void). Reste du fichier byte-identique au md5          │
-- │      2778691e (pt 19).                                      │
-- └──────────────────────────────────────────────────────────┘
...
BEGIN
  ...
  -- ────────────────────────────────────────────────────────
  -- B5 (D8) : coach retiré du domaine auth_roles (sql/63). La PORTE devient
  -- admin/bureau/referent ; le PÉRIMÈTRE catégoriel du referent est borné plus
  -- bas, bloc (1bis), AVANT tout INSERT (anti-effet de bord).
  IF NOT (has_role('admin') OR has_role('bureau') OR has_role('referent')) THEN
    RAISE EXCEPTION 'access denied: admin, bureau or referent role required (creer_evenement_complet)';
  END IF;
  ...
  -- ────────────────────────────────────────────────────────
  -- (1bis) BORNAGE CATÉGORIEL B5 (§4.2) — AVANT tout INSERT
  --     La PORTE est passée (admin/bureau/referent). Ici on borne le
  --     PÉRIMÈTRE : chaque équipe entrante (p_equipe_id scalaire +
  --     chaque p_equipes_engagees[].equipe_id) doit appartenir à une
  --     catégorie autorisée pour le compte courant. admin/bureau =
  --     transverses (puis_je_ecrire_categorie renvoie true) ; referent
  --     borné à ses catégories dérivées de fonction_staff (sql/64).
  --     Placé AVANT (2) -> aucun effet de bord si refus (anti-void,
  --     leçon REFONTE-EVT-write-M3/M5). Filtrage RPC = patron projet,
  --     reco P1 §4.2 (la RPC est SECURITY DEFINER, donc seul ce contrôle
  --     protège l'écriture — la RLS sous-jacente est bypassée).
  -- ────────────────────────────────────────────────────────

  -- (a) équipe scalaire (entrainement/stage ; competition mono éventuel)
  IF p_equipe_id IS NOT NULL THEN
    IF NOT public.puis_je_ecrire_categorie(public._b5_categorie_de_equipe(p_equipe_id)) THEN
      RAISE EXCEPTION 'access denied (B5) : catégorie de l''équipe % hors de votre périmètre autorisé', p_equipe_id
        USING ERRCODE = 'insufficient_privilege';
    END IF;
  END IF;

  -- (b) chaque équipe engagée (compétition multi-équipes M3)
  IF p_equipes_engagees IS NOT NULL THEN
    FOR v_engagement IN SELECT * FROM jsonb_array_elements(p_equipes_engagees) LOOP
      IF NOT public.puis_je_ecrire_categorie(
                public._b5_categorie_de_equipe((v_engagement->>'equipe_id')::uuid)) THEN
        RAISE EXCEPTION 'access denied (B5) : catégorie de l''équipe engagée % hors de votre périmètre autorisé', (v_engagement->>'equipe_id')
          USING ERRCODE = 'insufficient_privilege';
      END IF;
    END LOOP;
  END IF;

  -- NB club-wide (D9, point 4) : un évènement transverse « fête du club »
  -- (sans équipe -> sans catégorie) relève du chantier `Accès` (modélisation
  -- technique non définie). En l'état, un évènement competition à equipe_id
  -- NULL ET sans équipes engagées n'est pas un cas atteignable ici (la
  -- validation competition ci-dessus exige >=1 équipe engagée), donc aucun
  -- trou de bornage : tout évènement créé porte au moins une catégorie bornée.
