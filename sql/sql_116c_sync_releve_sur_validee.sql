-- =====================================================================
-- sql_116c_sync_releve_sur_validee.sql
-- Chantier : MISSION-TO-COUNTER (doctrine 2+3 — declaration -> validation)
-- Objet    : CREATE OR REPLACE de sync_releve_depuis_occurrence : la fusion
--            vers le compteur se declenche desormais sur l'etat 'validee'
--            (validation administrative), PAS sur 'realisee' (simple constat).
--            Une occurrence realisee mais non encore validee n'alimente PAS
--            le compteur (en attente de validation admin/bureau). Tout autre
--            etat (prevue|realisee|annulee) -> DELETE de la ligne de releve.
--            Le reste du corps (mapping, contrat, duree) est IDENTIQUE a sql_116b.
...
begin
  ...
  -- 2) Cas DELETE : occurrence non validee -> retirer la ligne de releve si presente.
  if v_occ.etat is distinct from 'validee' then
  ...
  -- 3) Cas UPSERT : occurrence 'validee' (validee par l'administration).
