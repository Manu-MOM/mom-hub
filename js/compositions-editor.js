...
(function () {
  ...
  function renderEditorTerrain(el, compo) {
    ...
    for (const code in TERRAIN_POS) {
      ...
      html += '<div class="vt-mark vt-drop vt-drag ' + cssClassEtatJoueur(etatDeriveJoueur(cj)) + '" draggable="true"' +
                ...
                '<span class="vt-mark__poste">' + escapeHtml(poste.libelle_court || poste.code) + '</span>' +
                '<span class="vt-mark__disc"><span class="vt-mark__num">' + escapeHtml(num) + '</span></span>' +
                (cj.est_capitaine ? '<span class="vt-mark__capitaine" title="Capitaine">C</span>' : '') +
  ...
  function renderSlotPoste(poste, numeroAffiche) {
    ...
    return (
      '<li class="slot slot--occupe ' + cssClassEtatJoueur(etatDeriveJoueur(cj)) + '" data-compo-joueur-id="' + escapeHtml(cj.id) + '" data-poste-id="' + escapeHtml(poste.id) + '">' +
        ...
        '<button class="slot__capitaine' + (cj.est_capitaine ? ' is-capitaine' : '') + '" title="' + (cj.est_capitaine ? 'Capitaine' : 'Désigner capitaine') + '" type="button">C</button>' +
  ...
  function bindSlotHandlers() {
    ...
    // Désignation du capitaine (bouton « C »). Un seul par compo, garanti côté
    // base (RPC atomique + index unique). Le clic bascule ce joueur capitaine.
    document.querySelectorAll('.slot--occupe .slot__capitaine').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
        const cjId = btn.closest('.slot').dataset.compoJoueurId;
        if (cjId) onCapitaineClick(cjId);
      });
    });
  }
  ...
  // Désigne le capitaine de la compo courante (bouton « C » d'un titulaire).
  // Atomique côté base (RPC definir_capitaine) : pose ce joueur capitaine et
  // retire l'ancien. renderEditorArea() re-rend la vue courante — liste OU
  // terrain (qui redessine le badge « C » et rejoue _injecterPhotosTerrain) —
  // donc la mise à jour est visible dans les deux vues.
  async function onCapitaineClick(compoJoueurId) {
    if (!State.selectedCompoId || !compoJoueurId) return;
    const r = await SupabaseHub.definirCapitaine(State.selectedCompoId, compoJoueurId);
    if (!r.ok) { alert('Désignation capitaine impossible : ' + r.error); return; }
    await loadCompoJoueurs();
    renderEditorArea();
  }
