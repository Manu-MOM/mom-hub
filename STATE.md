...
> - **`sql/C12-g-entete-rencontre.sql`** — RPC `get_entete_rencontre(p_token)` jeton-bornée RGPD-safe (libelle / type_competition / date_debut / adversaire_nom / domicile_exterieur + libellé site via LEFT JOIN `sites`). **Absorbe SUIVI-UI-1 ET SUIVI-UI-6** : le rôle du jeton est renvoyé en piggy-back (`role_lien`), zéro RPC dédiée. Posture grant = `get_compo_reduite_rencontre` (anon + authenticated). **Sous-point remonté, non décrété : la catégorie d'âge (« M14 ») n'est PAS incluse** — absente de `evenements`, exige la chaîne `equipe_uuid → equipes → ententes.categorie_id → categories` (4 sauts) + le schéma `categories` à vérifier à la source ; `type_competition` fourni en attendant. Décision Manu requise si le tampon doit afficher la catégorie d'âge.
...
| **SUIVI-UI-1** ✅ LIVRÉE | **RPC en-tête rencontre** — `get_entete_rencontre(p_token)` jeton-bornée RGPD-safe (libelle / type_competition / date_debut / adversaire_nom / domicile_exterieur + libellé site). **Livrée `sql/C12-g` (exécuté + committé 17/05).** 🟡 **Sous-point ouvert remonté (non décrété) : catégorie d'âge (« M14 ») NON incluse** — absente de `evenements`, exige la chaîne `equipe_uuid → equipes → ententes.categorie_id → categories` (4 sauts) + schéma `categories` à vérifier ; `type_competition` fourni en attendant. Décision Manu si le tampon doit afficher la catégorie d'âge | Ajout backend — **fait** | conv Production (couloir backend) — **clos** | ✅ Levée (hors sous-point catégorie, non bloquant) |
