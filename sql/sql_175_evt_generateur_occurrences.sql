-- =====================================================================
-- sql_175 — EVT-RECURRENCE-OCCURRENCES — Étape 2/4 : générateur d'occurrences
-- =====================================================================
-- Génère les occurrences (événements ENFANTS) d'un entraînement récurrent
-- hebdomadaire. Occurrences = vrais evenements avec evenement_parent_id.
--   - jour de la série = jour ISO de date_debut de la mère
--   - heure reprise de la mère (Europe/Paris)
--   - borne fin = recurrence.fin sinon fin de saison active
--   - saute la date de la mère elle-même (= 1re séance) et les dates_exclues
--   - idempotent : EXISTS sur (parent, date) — aucun doublon
--
-- Architecture : _core SANS garde (réutilisé par le trigger et le wrapper),
-- + wrapper public GARDÉ (admin|coach) qui délègue au _core.
-- (has_role() est faux en SQL Editor : valider le wrapper dans l'app.)
--
-- Exécuté et vérifié : occurrences toutes au bon jour, remontent en agenda.
-- =====================================================================
...
declare
  ...
  v_freq text; v_dow int; v_heure time; v_fin_rec date; v_saison_fin date;
...
begin
  ...
  v_heure := (v_mere.date_debut at time zone 'Europe/Paris')::time;
    ...
    if extract(isodow from v_d)::int = v_dow
      ...
      if exists (select 1 from public.evenements c
        ...
        insert into public.evenements (
          ...
          (v_d::timestamp + (v_heure - time '00:00')) at time zone 'Europe/Paris',
...
begin
  if not (has_role('admin') or has_role('coach')) then
    raise exception 'access denied: admin or coach role required (generer_occurrences_evenement)'
