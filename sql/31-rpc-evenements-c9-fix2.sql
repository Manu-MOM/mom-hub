-- =====================================================================
-- MOM HUB · sql/31-rpc-evenements-c9-fix2.sql
-- =====================================================================
...
-- Contexte
-- ─────────────────────────────────────────────────────────────────────
-- 2e bug identifié en S2.2 après le fix sql/30 (ajout evenement_parent_id) :
-- le déploiement inline du tournoi Challenge Vié affiche tous les 6 matchs
-- enfants sous une seule phase "(sans phase)" au lieu des 2 phases attendues
-- (Poule de brassage + Phases finales). Cause : les colonnes phase_libelle
-- et ordre_dans_phase ne sont PAS retournées par get_evenements_a_venir
-- et get_evenements_passes, alors qu'elles existent dans la table evenements
-- (cf. sql/10 §4.2).
--
-- Même schéma que sql/30 : oubli de colonnes dans le RETURNS TABLE.
--
-- ─────────────────────────────────────────────────────────────────────
-- Correctif
-- ─────────────────────────────────────────────────────────────────────
-- Ajout de 2 colonnes au RETURNS TABLE des 3 RPC liste :
-- - phase_libelle       TEXT
-- - ordre_dans_phase    INTEGER
--
-- - get_evenements_a_venir          : 18 → 20 colonnes
-- - get_evenements_passes           : 18 → 20 colonnes
-- - get_prochain_evenement_par_equipe : 18 → 20 colonnes (hérite via SELECT *)
--
-- get_evenement_with_encadrants : NON modifiée, retourne déjà ces 2 colonnes
-- (cf. sql/29, colonnes 17 et 18 du RETURNS TABLE).
--
-- ─────────────────────────────────────────────────────────────────────
-- Sécurité
-- ─────────────────────────────────────────────────────────────────────
-- Identique aux RPC sql/29 + sql/30 : LANGUAGE sql, SECURITY DEFINER,
...
-- =====================================================================
-- 1 · get_evenements_a_venir (MODIFIÉE — ajout phase_libelle + ordre_dans_phase)
-- =====================================================================
...
'Phase 4.2.B + C9-d + S2.2-fix2 — Liste événements à venir. 20 colonnes (ajout phase_libelle + ordre_dans_phase pour regroupement par phase des enfants de tournoi). Exclut annule/archive.';
...
'Phase 4.2.B + C9-d + S2.2-fix2 — Prochain événement (0 ou 1 ligne). 20 colonnes héritées via SELECT *.';


-- =====================================================================
-- 3 · get_evenements_passes (MODIFIÉE — ajout phase_libelle + ordre_dans_phase)
-- =====================================================================
...
'C9-a + S2.2-fix2 — Liste événements passés. 20 colonnes (ajout phase_libelle + ordre_dans_phase). Inclut annule, exclut archive.';
...
BEGIN
  ...
  RAISE NOTICE '────────────────────────────────────────────────────────';
  RAISE NOTICE '✅ S2.2-fix2 — sql/31';
  RAISE NOTICE '   get_evenements_a_venir cols : % (attendu 20)', v_avenir_cols;
  RAISE NOTICE '   get_evenements_passes  cols : % (attendu 20)', v_passes_cols;
  RAISE NOTICE '   get_prochain_event_par_eq   : % (attendu 20)', v_prochain_cols;
  RAISE NOTICE '────────────────────────────────────────────────────────';
END $$;
...
-- =====================================================================
--
-- Pour vérifier que les 2 nouvelles colonnes remontent bien :
--
--   SELECT code, libelle, phase_libelle, ordre_dans_phase
--   FROM get_evenements_a_venir('bfb83b83-83ef-4dde-b526-48ff87313044'::UUID, 90)
--   WHERE evenement_parent_id IS NOT NULL
--   ORDER BY phase_libelle, ordre_dans_phase;
--
-- Attendu : 6 lignes des matchs enfants du Challenge Vié
--           phase_libelle 'Poule de brassage' (3 lignes ordre 1-3)
--           phase_libelle 'Phases finales' (3 lignes ordre 1-3)
-- =====================================================================
