-- =====================================================================
-- sql_213_referent_de_categorie.sql
...
comment on function public.referent_de_categorie(uuid) is
  'PLANIF-FRISE-COSMETIQUE-EXPORT (sql_213) : referent(s) de categorie actif(s), resolus en prenom/nom, pour le cartouche d''export de la planification. Referent = fonction metier fonction_staff normalisee via _b5_norm (PAS une FK). SECURITY DEFINER (franchit la RLS fermee de fonction_staff), lecture seule, multi-referent (0..N lignes). Reutilise get_noms_personnes pour la resolution nominale.';
...
begin
  ...
  if v_secdef is null then
    raise exception 'sql_213 FAIL : fonction referent_de_categorie(uuid) absente.';
  end if;
  if v_secdef is not true then
    raise exception 'sql_213 FAIL : referent_de_categorie n''est pas SECURITY DEFINER.';
  end if;
  if v_provol <> 's' then
    raise exception 'sql_213 FAIL : referent_de_categorie n''est pas STABLE (provolatile=%).', v_provol;
  end if;
  ...
  if v_has_anon is true then
    raise exception 'sql_213 FAIL : anon a EXECUTE sur referent_de_categorie (fuite).';
  end if;
  if v_has_auth is not true then
    raise exception 'sql_213 FAIL : authenticated N''A PAS execute sur referent_de_categorie.';
  end if;

  -- (c) dependances presentes (echoue tot si le socle a bouge).
  if not exists (select 1 from pg_proc where proname = '_b5_norm') then
    raise exception 'sql_213 FAIL : helper _b5_norm absent.';
  end if;
  if not exists (select 1 from pg_proc where proname = 'get_noms_personnes') then
    raise exception 'sql_213 FAIL : RPC get_noms_personnes absent.';
  end if;

  raise notice 'sql_213 OK : referent_de_categorie(uuid) creee, SECURITY DEFINER, authenticated-only, dependances presentes.';
end;
$verif$;
