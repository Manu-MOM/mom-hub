-- =====================================================================
-- sql_179 — EVT-RECURRENCE-OCCURRENCES — correctif encadrants occurrences
-- =====================================================================
-- Le générateur _core copie désormais les ENCADRANTS (M8) de la mère sur
-- chaque occurrence créée. Sans ça, les occurrences n'avaient aucun
-- encadrant → vignette « Encadrement » affichée « à venir » (inaccessible)
-- alors que la mère avait bien son staff.
--
-- Réparation d'une série DÉJÀ générée (copier les encadrants de la mère
-- sur ses occurrences, sans doublon) :
--   insert into evenement_encadrants (evenement_id, personne_id, roles_encadrement, ordre, notes)
--   select o.id, ee.personne_id, ee.roles_encadrement, ee.ordre, ee.notes
--   from evenements o
--   cross join evenement_encadrants ee
--   where o.evenement_parent_id = <MERE_ID> and ee.evenement_id = <MERE_ID>
--     and not exists (select 1 from evenement_encadrants x
--                     where x.evenement_id=o.id and x.personne_id=ee.personne_id);
--
-- Exécuté et vérifié : série mardi Senior F réparée (47 occ × 2 = 94 liaisons).
-- Note : supprimer_occurrence_evenement purge déjà les encadrants de
-- l'occurrence supprimée (pas de liaisons orphelines).
-- =====================================================================
