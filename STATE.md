...
> **Restes Suivi UI : S-5.b** (Mode Vidéo S-5.3 + temps de jeu S-5.4 — **périmètre COACH authentifié, pas le bénévole sans login** : arbitrage de périmètre à poser, potentiellement hors `suivi.html`) puis **S-6** (synthèse & dettes, paquet sans code = la clôture). Voir dettes SUIVI-UI-4/5/6 ci-dessous.
...
| **Suivi de rencontre** | **Backend C12 déployé · UI module bénévole COMPLET (S-1→S-5.a)** | 🔧 MAJ Prod 17/05 — `suivi.html` + `js/suivi-client.js` v1.1 + `js/suivi-app.js` **v0.14**. Utilisable en live de bout en bout. Reste S-5.b (périmètre coach) + S-6 | 🟢/🟡 |
...
| **Suivis de Match** | `suivi.html` + `js/suivi-client.js` v1.1 + `js/suivi-app.js` **v0.14** | 🟡 **UI module bénévole COMPLET (S-1→S-5.a)** · ✅ backend C12 déployé · ⚪ conçu (UX S-1→S-6) | 🔧 MAJ Prod 17/05. Utilisable en live de bout en bout (entrée→coup d'envoi→saisie→correction→blessure→fin→reprise). Reste S-5.b (Mode Vidéo/temps de jeu = périmètre coach) + S-6 |
...
**STATE.md — version de référence (FUSION).** Reconstruite conv `Audits` → enrichie conv `Conception` 16/05 → repli DS-1 conv `Audits` 16/05 → MAJ Production 16/05 (Phase 5.14 + Phase 2) → MAJ Production 16/05 (C12) → MAJ Production 16/05 (Suivi UI S-1) → **MAJ Production 17/05 (Suivi UI S-2 → S-5.a) : module bénévole complet de bout en bout (suivi.html + suivi-client.js v1.1 + suivi-app.js v0.14) + dettes SUIVI-UI-4/5/6**.
