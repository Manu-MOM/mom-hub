-- =====================================================================
-- sql_116_sync_releve_depuis_occurrence.sql
-- Chantier : MISSION-TO-COUNTER (D7 — fusion occurrence -> compteur)
-- Objet    : RPC idempotente qui synchronise la ligne de
--            releve_heures_salarie avec l'etat courant d'une occurrence
--            de mission (mission_seances).
...
begin
  ...
  v_categorie := case v_type
    when 'intervention_ecole'     then 'seance'
    when 'entrainement_interne'   then 'seance'
    when 'entrainement_club'      then 'seance'
    when 'entrainement_simple'    then 'seance'
    when 'accompagnement_terrain' then 'seance'
    when 'stage'                  then 'seance'
    when 'reunion'                then 'reunion'
    when 'prepa'                  then 'formation'
