...
| **Prochaine conv métier** | Cycle `SUIVI-COACH-1` — A livré, **B conçu/validé + backend livré (C12-j)**, reste C | Production Objet B : câbler `SUIVI-COACH-3-auth` (seul verrou backend restant) puis coder l'écran ; Objet C en conv `Conception` → `Production` | ⚪ |
...
| **Suivis de Match** | `suivi.html` + `js/suivi-client.js` v1.1 + `js/suivi-app.js` **v0.14** | 🟢 **UI module bénévole CLÔTURÉ (S-1→S-6)** · ✅ backend C12 + dettes couloir backend déployés · ⚪ conçu (UX S-1→S-6) | 🔧 MAJ Prod 17/05. Module bénévole complet (entrée→coup d'envoi→saisie→correction→blessure→fin→reprise). **Couloir backend dettes SUIVI clos** (C12-g/h/i). Suite = cycle coach `SUIVI-COACH-1` Objets B/C, désormais débloqués (hors module bénévole). **Objet B (Mode Vidéo) conçu/validé 17/05 (conv `Conception`), reste C** |
...
│ └── C12-a→f (Suivi) + C12-g/h/i (dettes couloir backend SUIVI) + C12-j (SUIVI-COACH-3 chemin coach) — EXÉCUTÉS en base + rangés + committés
└── assets/
 └── logo-m2m.png ✅
```
...
3. **Cycle `SUIVI-COACH-1` Objets B/C** (conv `Conception` → `Production`) : module bénévole S-1→S-6 CLÔTURÉ, dettes backend SUIVI (C12-g/h/i) LIVRÉES, Objet A livré. **Objet B (Mode Vidéo) conçu/validé 17/05** (`Conception-SUIVI-COACH-1-ObjetB.md`, à déposer Manu). **17/05 : vérification source C12-c FAITE → `SUIVI-COACH-3` confirmée réelle et LIVRÉE (`sql/C12-j`, chemin coach-authentifié, pattern C12-i, C12-c non modifié).** → **conv `Production` Objet B** : (a) **câbler `SUIVI-COACH-3-auth`** (mapping `auth.uid()`→`personnes` du helper `_coach_personne_uuid()`, à vérifier à la source Phase 2.5 — seul verrou backend restant de B) ; (b) coder l'écran (fichier distinct, `supabase-client.js` coach) ; (c) spécifier en Production l'accroche dans `evenements-browser.js` (modèle SUIVI-COACH-2, zéro retouche logique d'A). Puis **Objet C** (temps de jeu C-1 + écran spectateur C-2) à concevoir (cycle suivant).
...
| **SUIVI-COACH-3-auth** 🆕 | **Câblage du mapping `auth.uid()` → `personnes.id` du coach** (helper `_coach_personne_uuid()` de `sql/C12-j`). Le lien entre la session Supabase Auth et la ligne `personnes` n'apparaît dans **aucune source lue** (C12-c, Modelisation-Evenements) ; le STATE mentionne « Auth Magic Link Phase 2.5 » sans exposer le mapping. **Discipline : non inventé** — isolé dans un **unique helper** qui lève une exception explicite tant que non câblé (procédé identique à `chronologie_nom_court_personne`/C12-f §3). À câbler en Production sur le pattern d'auth Phase 2.5 **vérifié à la source** (colonne `personnes.auth_user_id` ? table `auth_roles` ? autre — ne pas deviner). Tant que non câblé : le chemin coach (C12-j) échoue proprement ; **le chemin jeton bénévole de C12-c reste 100 % intact**. | Câblage technique (1 helper isolé) | conv Production | 🟡 Non bloquant pour l'existant (chemin bénévole intact) ; **bloque le couloir Production Objet B** (spec fine + écran Mode Vidéo) tant que non câblé |
