...
| **Prochaine conv métier** | Cycle `SUIVI-COACH-1` — A livré, **B conçu/validé**, reste C | Production Objet B (sous réserve vérif source C12-c) puis Objet C en conv `Conception` → `Production` ; pré-requis backend tombés | ⚪ |
...
3. **Cycle `SUIVI-COACH-1` Objets B/C** (conv `Conception` → `Production`) : module bénévole S-1→S-6 CLÔTURÉ, dettes backend SUIVI (C12-g/h/i) LIVRÉES, Objet A livré. **Objet B (Mode Vidéo) conçu/validé 17/05** (`Conception-SUIVI-COACH-1-ObjetB.md`, à déposer Manu) → **conv `Production` Objet B** : (a) vérifier à la source `sql/` C12-c (chemin coach-authentifié + `timecode_video`) → consommer si présent, ouvrir dette `SUIVI-COACH-3` si absent ; (b) coder l'écran (fichier distinct, `supabase-client.js` coach) ; (c) spécifier en Production l'accroche dans `evenements-browser.js` (modèle SUIVI-COACH-2, zéro retouche logique d'A). Puis **Objet C** (temps de jeu C-1 + écran spectateur C-2) à concevoir (cycle suivant).
